\\ Leg F: parametrize via z_2 = p/q instead of z_1.
\\ X̃ = (d_2 p^2 + e_2 q^2) / q^2. Check d_1*(X̃-e_1) and d_3*(X̃-e_3) are squares.
...
P_BOUND = 500000;
Q_BOUND = 500;
print("Leg F: z_2 = p/q with |p| <= ", P_BOUND, ", q <= ", Q_BOUND);
print();
...
for(k = 1, #triples,
  ...
  for(q = 1, Q_BOUND,
    ...
    for(p = 0, P_BOUND,
      if(found_this, break);
      if(gcd(p, q) != 1, next);
      \\ X̃ = (d2 p^2 + e2 q^2)/q^2
      \\ d_1*(X̃ - e1) = d_1*(d2 p^2 - C12*q^2)/q^2; integer iff num*d_1 is a perfect square in Z (since q^2 is sq).
      \\ Note: C12 = e1 - e2 negative, so X̃ - e1 = (d2 p^2 - C12 q^2)/q^2 = (d2 p^2 + (e2-e1) q^2)/q^2 ≥ 0 for d_2>0, fine.
      val1 = d1 * (d2*p^2 - C12*q^2);
      if(val1 < 0, next);
      if(!issquare(val1), next);
      val3 = d3 * (d2*p^2 - C32*q^2);
      if(val3 < 0, next);
...
t_end = getwalltime();
print("=== Leg F complete ===  wall = ", (t_end - t_start)/1000.0, "s  hits = ", hits);
quit;
